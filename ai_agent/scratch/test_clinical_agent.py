...
def run_verification():
    ...
    # Check if OpenRouter/Gemini API key exists, otherwise mock the output
    has_api_key = bool(
        os.getenv("OPENROUTER_API_KEY") or os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    )

    if not has_api_key:
        print("[!] No API key detected. Using Mock Client for test.")
