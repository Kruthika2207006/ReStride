...
1. Patient Request Profile:
- Age: {request.age}
- Weight: {request.weight_kg}
- Activity level: {request.activity_level}
- Amputation: {request.amputation_level}

2. Geometry Agent:
- Shape: {geometry.get("shape_descriptor")}
- Length: {geometry.get("limb_length_cm")} cm
- Volume: {geometry.get("volume_cm3")} cm3
- Bounding Box: {geometry.get("bounding_box_dims")}
- Circumferences: {geometry.get("cross_sectional_circumferences")}
- Watertight: {geometry.get("is_watertight")}

3. Clinical Reasoning Agent:
- Summary: {clinical.get("limb_analysis_summary")}
- Socket Type: {clinical.get("recommended_socket_type")}
- Reasoning: {clinical.get("recommended_socket_type_reasoning")}
- Sensitive Areas: {clinical.get("pressure_sensitive_regions")}
- Design Considerations: {clinical.get("design_considerations")}

4. Socket Recommendation Agent:
- Socket Design: {socket_rec.get("socket_design_type")}
- Design Reasoning: {socket_rec.get("socket_design_reasoning")}
- Suspension System: {socket_rec.get("suspension_system")}
...
"""
        ...
        # Generate structured final consensus response from Gemini
